...
**1. `EvalOptions::default()` is `global: true` — script mode.** The obvious `ctx.eval(src)` therefore
rejects an ES-module guest (`export default {`) on its first character. `global: false` selects
`JS_EVAL_TYPE_MODULE`, but **`eval_with_options` in module mode returns the module's *evaluation
promise*, which resolves to `undefined`** — the namespace is unreachable from that entry point
entirely. The door that works is `Module::declare(ctx, name, src)?.eval()?` followed by
`get::<_, Value>("default")`, and it also compiles *without running*, which is what a `compile()` gate
wants. `EvalOptions` is `#[non_exhaustive]`, so it cannot be built with a struct literal from another
crate — default it and mutate the field.

**2. `Context::with` holds the runtime's global lock for the whole closure** (`context/base.rs:109`).
Under the `parallel` feature, `Runtime::is_job_pending` and `Runtime::execute_pending_job` each take
that same non-reentrant lock (`runtime/base.rs:162,170`) — so calling either **from inside a context
scope deadlocks the thread.** Use `Ctx::execute_pending_job` (`context/ctx.rs:375`), which calls
`JS_ExecutePendingJob` directly with no lock. Cost: the `Ctx` variant returns `bool` and **folds "a job
ran" together with "a job threw"** (both are `res != 0`), so a job's exception must be read off the
promise, not the return value.

**3. `Send`/`Sync` for `Runtime`/`Context` exist only under `cfg(feature = "parallel")`**
(`runtime/base.rs:187-197`, `context/base.rs:144-149`). Without it both are `!Send`, and a trait
declared `Send + Sync` cannot hold one. With it, bite 2 applies. **The feature a `Send + Sync` seam
...
- `Value::into_object()` / `into_array()` / `into_promise()` return **`Option`**; the `Result`-returning
  spellings are `try_into_*`. `into_promise()` is how you get a `Promise` back from a `Value`.
- `Promise::state()` → `PromiseState` (`Pending`/`Resolved`/`Rejected`); `result::<T>()` →
  `Option<Result<T>>`. `Promise::new(ctx)` / `Ctx::promise()` → `(Promise, resolve, reject)`.
- `Module::evaluate` / `Module::eval` return a **promise** alongside the module, because module bodies
  may use top-level await — observe it even when the guest has none.
- `rquickjs::context::EvalOptions` and `rquickjs::promise::PromiseState` are the paths; the root
  re-exports do not include them.
- Returning a `Promise` from a `Function::new` host closure works — the conversions are **generated by
  the `sub_types!` macro** (`value.rs:707-717`, `Promise->Object->Value as_promise … into_promise …`).

### Measurement discipline (the two that mattered most)

**A probe pass with an impossible timing is not a pass.** A probe claiming "the interrupt handler
aborts a spinning guest" *passed* reporting an abort after **23 µs**. The guest never ran — an earlier
step was failing instantly. The claimed mechanism cannot act in 23 µs, and that discrepancy was the
only clue; fixing the setup produced the real figure, **300.7 ms against a 300 ms budget**. A green
result inconsistent with the mechanism it claims to exercise is measuring something else. Symmetric
rule to "prove a test fails before trusting it passes".

**A text-search absence claim can be false when a macro generates the code.** A search for
`IntoJs for Promise` found nothing, nearly concluding a promise cannot cross the host boundary; the
impl is macro-generated and does not exist as text. This is a *different* failure from the bash-`grep`
shim — the instrument is fine, the code simply is not there to find. **Absence is a claim about
behaviour, and generated code has behaviour without text.**

**An `unsafe impl Send` is a claim, and the `cfg` gate is the answer.** A hypothesis that "a QuickJS
runtime is `!Send`, so the seam forbids the obvious struct" was contradicted by the source in one
grep — `unsafe impl Send for Runtime` exists. The real answer was one level down: it is
`#[cfg(feature = "parallel")]`. Being wrong in that direction mattered twice, because enabling the
feature is what causes bite 2.

### Environment

`cargo fetch` works through the ambient proxy. **A `403` from `crates.io`'s API is not a signal that
the fetch will fail** — the sparse index (`https://index.crates.io/<ab>/<cd>/<crate>`) returned `200`
for the same crates and the fetch succeeded. Four of the six proxy vars are set here
(`HTTP_PROXY`/`HTTPS_PROXY`/`http_proxy`/`https_proxy` → `127.0.0.1:55769`). `rquickjs 0.9` with the
`bindgen` feature builds cleanly on this machine, so libclang is present.

**Bash `grep` lied twice more in this session** — empty output on `^impl` for a file with `impl` at
column 0, and on `^## ` for a file with headings. The Grep tool agreed with the file every time.

### A gate's directory scope is a blind spot for new top-level directories

`scripts/key-leak-grep.sh` scans `apps packages scripts`. Content added anywhere else — including
tracked directories such as `.workbuddy-ai/` — is **outside the guard**. Widening it is a
