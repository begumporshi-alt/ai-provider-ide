...
- Absence claims: **Grep tool** (bash `grep` shim lies) — **skips dot-dirs** (`.github/`, `.workbuddy-ai/` → `cat <dir>/* | grep`). A hit ≠ completeness; count via node, not `find | grep -c`.
...
- **The bulk-delete guard is sandbox-only.** A Playwright `webServer` timeout is a guard symptom **or a proxy `502`** (only the 60s timeout surfaces). Clean in `web-test:clean`, *after* `pnpm build`; `DEBUG=pw:webserver` tells `ECONNREFUSED` from a non-2xx.
...
**Tauri boundary** — A `#[tauri::command]` arg, a serde field and the Rust→webview `BridgeRequest` are **different boundaries**, and **a field on one side is not wiring**; only nested payloads hit serde → give every nested payload `deny_unknown_fields`; `shim.ts:toRustArgs` snake_cases top-level keys → `REFERENCE.md`. **Every new command needs a `web-test/shim.ts` case the same day** → `REFERENCE.md`. A tool failure must never reach the model as `""` — guard at bridge *and* consumer, and clear stale data on failure.
