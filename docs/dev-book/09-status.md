...
| **Headless Phase 2 — the router core in Rust** | **Increments 1–3 landed 2026-09-23.** `core/engine.rs` holds the port's pure core: the error taxonomy, `COOLDOWN_FLOOR_MS`, the shortest-wait fold, and `HealthTracker` — so the floor the tracker *enforces* and the wait the client is *told* share one constant. `Bridge::dispatch` now takes a `ReplyHandle` instead of the bridge reaching back for the core, so a Rust bridge can answer at all and the core↔bridge cycle is unrepresentable. 25 tests (3 ported from `retry-after.test.ts`), `cargo test` 514/0, twelve falsifications fire. **Still parked:** the attempt loop, which needs `route_planner`'s `Candidate` (Phase 3), and the streaming half, which needs a `Stream` adapter for `chunks: AsyncIterable<string>`. Scoped in [10](10-headless-service.md) §7 — the port is 1,114 lines, not the 229 the plan implied |
