...
| **D4** | "per-app keys + budgets are the follow-up" | `ARCHITECTURE.md:848-850` | Half true. Per-app keys shipped. The spend cap is **global and monthly** — `gateway_spend_status` compares one `month_micros` against one `cap_micros`, with no per-key dimension. Per-app budgets are still open | **Half** | **Fixed** (per-app budgets remain open) |
