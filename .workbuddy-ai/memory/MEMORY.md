...
- **`apps/desktop/e2e/` is dead code.** `playwright.config.ts` sets `testDir: "./web-test"`, and nothing references the `e2e/` dir — not `package.json`, not ci.yml. Its four specs (`acceptance`, `code-adapter`, `drift-repair`, `onboarding`) never run.
