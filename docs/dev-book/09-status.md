...
| **The adapter runtime gets a phase, and increment 15 lands its I/O-free half** (2026-09-24) | D26's causal half is closed: §7 gains **Phase 4b**, and the number is lettered rather than renumbered because [11](11-cross-platform-tech-choice.md)'s Phase 5 is the same "delete the bridge" step and two documents share the numbering. **The two implementors of the seam are not equally load-bearing, and that was measured rather than assumed**: `adapter-runtime.ts:52-58` branches on `kind === "code"`, and the installed database holds **3 of 3 declarative and 0 of 3 code** — every builtin template is declarative too. Tier 2 is reachable but human-gated (`Onboarding.tsx:240-242`: "the last resort … an explicit human action, never an automatic fallback"), so the interpreter is the critical path and the sandbox is the exception path — which is why §2.1.3's `SIGSEGV` is a recorded residual risk rather than a blocker. **Increment 15** lands the I/O-free half in three modules, 66 tests, Rust **746 → 812**: `core/jsonpath.rs` (the 77-line source), `core/template.rs` (the required-placeholder versus omit-when-absent rule), and `core/manifest.rs` (headers and the sentinel, streaming tool-call reassembly, `read_cached_tokens`, `ManifestHttpError`). Three states were **not** redefined because the crate already has them — the sentinel is `egress::SENTINEL`, the mid-stream kind is `engine::FailureKind`, and a real tool call is `adapter::ToolCall`. **`modality.ts` is deferred on a dependency, not on effort**: `modelIdPattern` is a regular expression and this crate has no `regex` in its *runtime* graph — `Cargo.lock` lists `regex 1.13.1` only through `tauri-build`'s **build** graph, where `cargo tree -e normal -i regex --no-default-features` prints *nothing to print*. See §10 decision 5. Remaining in the phase: the interpreter's I/O half over `HttpPort`, then `modality.rs`, then the sandbox |
